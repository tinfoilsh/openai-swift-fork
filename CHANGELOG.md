...
### Added
- Provider-specific speech voices through `AudioSpeechVoice.custom(String)`, optional `streamFormat: .audio`, and `AudioSpeechStreamOptions.expectedContentType` on concrete `OpenAI` callback and async streaming overloads. Existing speech initializers and built-in voice enumeration are preserved.
...
### Changed
- **Source compatibility:** `AudioSpeechVoice` gains `.custom(String)` and `AudioSpeechQuery.CodingKeys` gains `.streamFormat`. Exhaustive switches over either enum must handle the new case or use a `default` arm. These additive cases are recorded in `.github/api-breakage-allowlist.txt`. Unknown voice strings now round-trip instead of failing to decode, and custom model identifiers are forwarded rather than replaced with `tts-1`.
- Binary speech streaming requires a successful HTTP response with an audio MIME type or `application/octet-stream`; callers can require a specific type such as `audio/pcm`. Missing or incompatible content types and empty responses fail. HTTP error bodies are bounded to 64 KiB.
- **Breaking (fork):** Three public initializers gained optional parameters that this fork added before the API-breakage gate existed, and are now recorded in `.github/api-breakage-allowlist.txt`: `Configuration.init` gained `tokenProvider:` after `token:` (dynamic bearer tokens), `ChatQuery.init` gained a trailing `extraBody:` (vendor extension fields on the request), and `ChatQuery.ChatCompletionToolParam.FunctionDefinition.init` gained a trailing `extra:` (vendor extension fields on a tool definition). Every new parameter defaults to `nil`, so call sites compile unchanged; only code that stores one of these initializers as a function value by its full signature is affected.
- **Breaking:** `Components.Schemas.ResponseErrorCode` is no longer `@frozen` or `CaseIterable` and gains an `other(String)` case. A `response.failed` event carrying a code outside the spec's list (OpenAI adds codes without a spec bump; OpenAI-compatible servers emit their own, such as `upstream_error`) used to fail to decode, hiding the error message behind a `DecodingError`; such codes now decode as `.other(code)`. `init(rawValue:)` is non-failable. Migration: exhaustive `switch` statements over the code need a `case .other` or `default` arm; pattern matches on named cases are unchanged. Accepted in `.github/api-breakage-allowlist.txt`.
...
### Fixed
- Speech chunks are delivered before completion on the session's serial queue, without treating binary audio as JSON. Speech failure and cancellation release the underlying request, complete once, and suppress queued audio, including cancellation from an audio callback.
