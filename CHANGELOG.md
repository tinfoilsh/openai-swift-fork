...
### Changed
- **Breaking (fork):** Three public initializers gained trailing optional parameters that this fork added before the API-breakage gate existed, and are now recorded in `.github/api-breakage-allowlist.txt`: `Configuration.init` gained `tokenProvider:` (dynamic bearer tokens), `ChatQuery.init` gained `extraBody:` (vendor extension fields on the request), and `ChatQuery.ChatCompletionToolParam.FunctionDefinition.init` gained `extra:` (vendor extension fields on a tool definition). Every new parameter defaults to `nil`, so call sites compile unchanged; only code that stores one of these initializers as a function value by its full signature is affected.
